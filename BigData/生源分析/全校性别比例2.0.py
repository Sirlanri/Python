...
def getNum(ExcelName):
    global allNum,seriseNames,girlNum,boyNum
    ...
    for sheet in allSheet:
        ...
        allPercent[sheet]=round(boyNum/girlNum,4)
...
def do_sex():
    global allPercent
    getNum('各学院.xlsx')
    for key in allPercent:
        print(key+str(allPercent[key]))
...
def vision_bar():
    c = (
        Bar()
        .add_xaxis(list(allPercent.keys()))
        .add_yaxis("男女比例", list(allPercent.values()))
        
        .set_global_opts(title_opts=opts.TitleOpts(title="18级男女比例（男/女）", subtitle="by 深蓝之邃"))
    )
    c.render(path='性别比例直方图.html')
do_sex()
vision_bar()